# @equixswap/token-lists (beta)

Cloned from [@uniswap/token-lists (beta)](https://github.com/equiXswap/token-lists/tree/v1.0.0-beta.28)

[![Tests](https://github.com/equiXswap/token-lists/workflows/Tests/badge.svg)](https://github.com/equiXswap/token-lists/actions?query=workflow%3ATests)
[![npm](https://img.shields.io/npm/v/@equixswap/token-lists)](https://unpkg.com/@equixswap/token-lists@latest/)

This package includes a JSON schema for token lists, and TypeScript utilities for working with token lists.

The JSON schema represents the technical specification for a token list which can be used in a dApp interface, such as the equiXswap Interface.

## What are token lists?

equiXswap Token Lists is a specification for lists of token metadata (e.g. address, decimals, ...) that can be used by any dApp interfaces that needs one or more lists of tokens.
...
Token list JSON must validate against the [JSON schema](https://json-schema.org/) in order to be used in the equiXswap Interface.
Tokens on token lists, and token lists themselves, are tagged so that users can easily find tokens.

## JSON Schema $id

The JSON schema ID is [https://equixswap.org/tokenlist.schema.json](https://equixswap.org/tokenlist.schema.json)
...
import { schema } from '@equixswap/token-lists'
import Ajv from 'ajv'
...
```typescript
import { TokenList, schema } from '@equixswap/token-lists'
...
A snapshot of the equiXswap default list encoded as a token list is found in [test/schema/bigexample.tokenlist.json](test/schema/bigexample.tokenlist.json).
